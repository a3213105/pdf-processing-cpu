...
def read_config():
    CONFIG_FILE_NAME: str = os.getenv('MINERU_TOOLS_CONFIG_JSON', 'magic-pdf.json')
    ...
    with open(config_file, 'r', encoding='utf-8') as f:
        config = json.load(f)
        config['models-dir'] = f'{home_dir}/{config["models-dir"]}'
        config['layoutreader-model-dir'] = f'{home_dir}/{config["layoutreader-model-dir"]}'
    return config
