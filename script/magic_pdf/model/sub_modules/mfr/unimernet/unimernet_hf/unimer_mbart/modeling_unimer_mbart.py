...
class UnimerMBartAttention(nn.Module):
    ...
    def enc_key_values(self, encoder_hidden_states):
        key_states = self._shape_qk(self.k_proj(encoder_hidden_states), -1, encoder_hidden_states.shape[0])
        value_states = self._shape_v(self.v_proj(encoder_hidden_states), -1, encoder_hidden_states.shape[0])
    ...
    def forward1(
        ...
        enc_past_key_value: Optional[Tuple[torch.Tensor]] = None,
...
class UnimerMBartDecoderLayer(nn.Module):
    ...
    def enc_key_values(self, encoder_hidden_states: torch.Tensor) :
        return self.encoder_attn.enc_key_values(encoder_hidden_states)
    ...
    def forward(
        ...
        if enc_past_key_value is not None:
            residual = hidden_states
            hidden_states = self.encoder_attn_layer_norm(hidden_states)
            hidden_states = self.encoder_attn.forward1(
                hidden_states=hidden_states,
                attention_mask=encoder_attention_mask,
                layer_head_mask=cross_attn_layer_head_mask,
                past_key_value=enc_past_key_value,
                output_attentions=output_attentions,
            )
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states
...
class UnimerMBartDecoder(UnimerMBartPreTrainedModel):
    ...
    def forward(
        ...
        enc_past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            enc_past_key_value = enc_past_key_values[idx] if enc_past_key_values is not None else None
            ...
            layer_outputs = decoder_layer(
                ...
                past_key_value=past_key_value,
                enc_past_key_value=enc_past_key_value,
                output_attentions=output_attentions,
                use_cache=use_cache,
            )
            hidden_states = layer_outputs[0]
...
class UnimerMBartForCausalLM(UnimerMBartPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        enc_past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        ...
        if enc_past_key_values is not None:
