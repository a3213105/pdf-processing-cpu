    ...
    for para_block in paras_of_layout:
        ...
        if para_type in [BlockType.TEXT, BlockType.LIST, BlockType.INDEX]:
            ...
            if mode == MakeMode.NLP_MD:
                ...
                # Check if image footnotes exist
                has_image_footnote = any(block['type'] == BlockType.IMAGE_FOOTNOTE for block in para_block['blocks'])
                # If there is a picture footnote, the picture footnote will be spliced ​​to the back of the picture body.
                if has_image_footnote:
                    for block in para_block['blocks']:  # 1st.spell image_caption
                        if block['type'] == BlockType.IMAGE_CAPTION:
                            para_text += merge_para_with_text(block) + '  \n'
                    for block in para_block['blocks']:  # 2nd.spell image_body
                    ...
                    for block in para_block['blocks']:  # 3rd.spell image_footnote
                        if block['type'] == BlockType.IMAGE_FOOTNOTE:
                            para_text += '  \n' + merge_para_with_text(block)
                else:
                    for block in para_block['blocks']:  # 1st.spell image_body
                        if block['type'] == BlockType.IMAGE_BODY:
                            for line in block['lines']:
                                for span in line['spans']:
                                    if span['type'] == ContentType.IMAGE:
                                        if span.get('image_path', ''):
                                            para_text += f"![]({img_buket_path}/{span['image_path']})"
                    for block in para_block['blocks']:  # 2nd.spell image_caption
                        if block['type'] == BlockType.IMAGE_CAPTION:
                            para_text += '  \n' + merge_para_with_text(block)
            ...
            if mode == MakeMode.NLP_MD:
                ...
                for block in para_block['blocks']:  # 1st.spell table_caption
                    if block['type'] == BlockType.TABLE_CAPTION:
                        para_text += merge_para_with_text(block) + '  \n'
                for block in para_block['blocks']:  # 2nd.spell table_body
                            ...
                            for span in line['spans']:
                                if span['type'] == ContentType.TABLE:
                                    ...
                                    if span.get('html', ''):
                                        ...
                                        para_text += f"![]({img_buket_path}/{span['image_path']})"
                for block in para_block['blocks']:  # 3rd.spell table_footnote
    ...
    for i, line in enumerate(para_block['lines']):
        ...
        for j, span in enumerate(line['spans']):
            ...
            if content:
                ...
                # Define CJK language collection (Chinese, Japanese and Korean)
                cjk_langs = {'zh', 'ja', 'ko'}
                # logger.info(f'block_lang: {block_lang}, content: {content}')

                # Determine whether it is span at the end of the line
                is_last_span = j == len(line['spans']) - 1

                if block_lang in cjk_langs: # In Chinese/Japanese/Korean context, line breaks do not require space separation.,But if it ends with an inline formula, you still need to add a space.
                    if is_last_span and span_type not in [ContentType.INLINE_EQUATION]:
                        para_text += content
                    else:
                        para_text += f'{content} '
                else:
                    # In the context of Western texts The last span of each line determines whether to remove hyphens
                    if span_type in [ContentType.TEXT, ContentType.INLINE_EQUATION]:
                        # If span is the last of a line and has a hyphen at the end, there should be no space at the end,At the same time, - should be deleted
                        if (
                                is_last_span
                            ...
                            if (
                                ...
                                para_text += content[:-1]
                            else:  # If there is no next line, or the first span of the next line does not start with a lowercase letter, keep the hyphen but no spaces.
                                para_text += content
                        else:  # In the context of Western texts contentNeed to be separated by spaces
                            para_text += f'{content} '
            else:
                continue
...
def escape_special_markdown_char(content):
    """
    Escape characters in the text that have special meaning to markdown syntax
    """
    special_chars = ["*", "`", "~", "$"]
