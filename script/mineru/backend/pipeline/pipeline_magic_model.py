...
class MagicModel:
    """Each function returns an empty list when no elements are obtained.."""
    def __init__(self, page_model_info: dict, scale: float):
        self.__page_model_info = page_model_info
        self.__scale = scale
        """Add bbox information (scale, poly->bbox)"""
        self.__fix_axis()
        """Delete model data with particularly low confidence (<0.05),improve quality"""
        self.__fix_by_remove_low_confidence()
        """Delete high iou(>0.9)The one with lower confidence in the data"""
        self.__fix_by_remove_high_iou_and_low_confidence()
        """Correct part of tbale_footnote to image_footnote"""
        self.__fix_footnote()
        """Handling overlapping image_body and table_body"""
    ...
    def __fix_by_remove_overlap_image_table_body(self):
                ...
                for j in range(i + 1, len(blocks)):
                    ...
                    if overlap_box is not None:
                        ...
                        if block_to_remove not in need_remove_list:
                            # Expand the bounding box of large blocks
        ...
        # Processing images - image overlap
        add_need_remove_block(image_blocks)
        # Handling tables - table overlaps
        add_need_remove_block(table_blocks)

        # Remove the marked block from the layout
    ...
    def __tie_up_category_by_distance_v3(self, subject_category_id, object_category_id):
        # Define functions to obtain subject and object objects
        def get_subjects():
    ...
    def get_equations(self) -> tuple[list, list, list]:  # There are coordinates and words
    ...
    def get_discarded(self) -> list:  # Self-developed model, only coordinates
        blocks = self.__get_blocks_by_type(CategoryId.Abandon)
        return blocks

    def get_text_blocks(self) -> list:  # This is a self-developed model with only coordinates and no words.
        blocks = self.__get_blocks_by_type(CategoryId.Text)
        return blocks

    def get_title_blocks(self) -> list:  # Self-developed model, only coordinates, no words
