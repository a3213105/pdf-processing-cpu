...
def projection_by_bboxes(boxes: np.array, axis: int) -> np.ndarray:
    """
     through a group bbox Obtain the projected histogram and finally end with per-pixel Formal output

    Args:
        boxes: [N, 4]
        axis: 0-xThe coordinates are projected horizontally, 1-yCoordinates are projected vertically

    Returns:
        1D Projection histogram, the length is the maximum value of the projection direction coordinate (we don’t need the actual side length of the picture, because we just want to find the interval of the text box)
...
def recursive_xy_cut(boxes: np.ndarray, indices: List[int], res: List[int]):
    ...
    Args:
        boxes: (N, 4)
        indices: During recursion, always means box index in original data
        res: Save output

    """
    # Towards y axial projection
    ...
    for r0, r1 in zip(arr_y0, arr_y1):
        # [r0, r1] Indicates that it is divided horizontally, with bbox areas, these areas will be divided vertically
        ...
        arr_x0, arr_x1 = pos_x
        if len(arr_x0) == 1:
            # x The direction cannot be divided
            res.extend(x_sorted_indices_chunk)
            continue

        # x can be separated in the direction and continue to call recursively
