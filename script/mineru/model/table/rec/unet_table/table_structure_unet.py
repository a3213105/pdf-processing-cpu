...
class TSRUnet:
    ...
    def postprocess(self, img, pred, **kwargs):
        ...
        )  # Whether to perform closure operations to find more small boxes
        more_h_lines = (
            kwargs.get("more_h_lines", enhance_box_line) if kwargs else enhance_box_line
        )  # Whether to adjust to find more horizontal lines
        more_v_lines = (
        ...
        )  # Whether to extend the line segment so that the endpoints are connected
        # Whether to perform rotation correction
        rotated_fix = kwargs.get("rotated_fix") if kwargs else True
        ori_shape = img.shape
        pred = np.uint8(pred)
        hpred = copy.deepcopy(pred)  # horizontal line
        vpred = copy.deepcopy(pred)  # vertical line
        ...
        )  # The process of expansion first and then corrosion
        if morph_close:
            hpred = cv2.morphologyEx(hpred, cv2.MORPH_CLOSE, hkernel, iterations=1)
        colboxes = get_table_line(vpred, axis=1, lineW=col)  # vertical line
        rowboxes = get_table_line(hpred, axis=0, lineW=row)  # horizontal line
    ...
    def cal_region_boxes(self, tmp):
        labels = measure.label(tmp < 255, connectivity=2)  # 8Connected region markers
    ...
    def cal_rotate_angle(self, tmp):
        # Calculate the outermost rotation box
    ...
    def unrotate_polygons(
        ...
        # Rotate polygon back to original position
        (h, w) = img_shape
        center = (w // 2, h // 2)
        M_inv = cv2.getRotationMatrix2D(center, -angle, 1.0)

        # Will (N, 8) Convert to (N, 4, 2)
        polygons_reshaped = polygons.reshape(-1, 4, 2)

        # Batch reverse rotation
        unrotated_polygons = cv2.transform(polygons_reshaped, M_inv)

        # Will (N, 4, 2) Convert back (N, 8)
