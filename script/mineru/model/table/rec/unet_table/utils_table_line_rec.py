...
def get_table_line(binimg, axis=0, lineW=10):
    ...
    labels = measure.label(binimg > 0, connectivity=2)  # 8Connected region markers
...
def calculate_center_rotate_angle(box):
    """
    around cx,cypoint w,h rotate angle coordinates,It can alleviate the internal tilt of the image to a certain extent, but it still depends on the model for stability.
    x = cx-w/2
    y = cy-h/2
...
def _order_points(pts):
    ...
    """
    ---------------------
    In this project, the purpose is to obtain after sorting[(xmin,ymin),(xmax,ymin),(xmax,ymax),(xmin,ymax)]
    Author: Tong_T
    Source: CSDN
    Original text: https://blog.csdn.net/Tong_T/article/details/81907132
    ...
    for i in range(lines_n):
        ...
        for j in range(lines_n):
            if i != j:
                ...
                ):  # Determine whether the projections of two horizontal lines in the y direction overlap
                    continue
                else:
                    r = sqrt((x1, y1), (x3, y3))
...
def line_to_line(points1, points2, alpha=10, angle=30):
    ...
    if (flag1 > 0 and flag2 > 0) or (flag1 < 0 and flag2 < 0):  # The horizontal or vertical line is on the same side of the vertical or horizontal line
        if (A1 * B2 - A2 * B1) != 0:
            x = (B1 * C2 - B2 * C1) / (A1 * B2 - A2 * B1)
            y = (A2 * C1 - A1 * C2) / (A1 * B2 - A2 * B1)
            # x, y = round(x, 2), round(y, 2)
            p = (x, y)  # Intersection point of horizontal and vertical lines
            r0 = sqrt(p, (x1, y1))
            r1 = sqrt(p, (x2, y2))

            if min(r0, r1) < alpha:  # If the distance between the intersection point and the starting point or end point of the line is less than alpha, extend the line to the intersection point
                if r0 < r1:
...
def point_line_cor(p, A, B, C):
    ##Determine the positional relationship between points and lines
