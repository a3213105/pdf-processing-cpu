...
def calculate_iou(
    ...
    # If there is no intersection, exit the detection directly.
    ...
    # Avoid divide-by-zero errors if the area is small enough that the product is 0,If it is considered to be a misidentification, remove it directly.
    if u_area == 0:
        return 1
...
def is_box_contained(
    ...
    if b1_x2 < b2_x1 or b1_x1 > b2_x2 or b1_y2 < b2_y1 or b1_y1 > b2_y2:
        return None
    # Calculate the total area of ​​box2
    b2_area = (b2_x2 - b2_x1) * (b2_y2 - b2_y1)
    b1_area = (b1_x2 - b1_x1) * (b1_y2 - b1_y1)
    ...
    # Calculate the ratio of the outside area to the total area of ​​box2
    ...
    if ratio_b2 < threshold:
        return 2
    # Determine whether the proportion is greater than the threshold
    return None
...
def sorted_ocr_boxes(
    ...
    threahold = 20
    # Avoid inconsistency between output and input formats and incompatibility with function functions
...
def plot_html_table(
    ...
    for point in logi_points:
        max_row = max(max_row, point[1] + 1)  # Add 1 because the closing subscript is included
        max_col = max(max_col, point[3] + 1)  # Add 1 because the closing subscript is included

    # Create a two-dimensional array to store sorted_logi_points elements in
    grid = [[None] * max_col for _ in range(max_row)]

    valid_start_row = (1 << 16) - 1
    valid_start_col = (1 << 16) - 1
    valid_end_col = 0
    # Will sorted_logi_points Fill in the elements in grid middle
