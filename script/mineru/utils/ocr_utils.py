...
LINE_WIDTH_TO_HEIGHT_RATIO_THRESHOLD = 4  # Generally speaking, a normal horizontal text block is only when the line width exceeds 4 times the height.
...
def _is_overlaps_y_exceeds_threshold(bbox1,
    ...
    """Check whether the two bboxes overlap on the y-axis, and the height of the overlapping area accounts for more than 80% of the lower height of the two bboxes."""
    _, y0_1, _, y1_1 = bbox1
    _, y0_2, _, y1_2 = bbox2
...
def _is_overlaps_x_exceeds_threshold(bbox1,
                                     bbox2,
                                     overlap_ratio_threshold=0.8):
    """Check whether two bboxes overlap on the x-axis, and the width of the overlapping area accounts for the lower width of the two bboxes exceeding the specified threshold"""
    x0_1, _, x1_1, _ = bbox1
    x0_2, _, x1_2, _ = bbox2
...
def merge_det_boxes(dt_boxes):
    ...
    for line in lines:
        ...
        # Calculate the width and height of the entire row
        ...
        # Merge only if row width exceeds 4 times height
        if line_width > line_height * LINE_WIDTH_TO_HEIGHT_RATIO_THRESHOLD:
            ...
            # Without merging, add the original area directly
...
def get_ocr_result_list(ocr_res, useful_list, ocr_enable, bgr_image, lang):
    ...
    for box_ocr_res in ocr_res:
        ...
        if len(box_ocr_res) == 2:
            ...
            if score < OcrConfidence.min_confidence:  # Filter low confidence results
                continue
        else:
        ...
        if calculate_is_angle(poly):
            ...
            # The angle with the x-axis exceeds 0.5degree, make some corrections to the boundaries
