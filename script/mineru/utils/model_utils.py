...
def remove_overlaps_min_blocks(res_list):
    ...
    for i in range(len(res_list)):
        # If the current element is already in the list that needs to be removed, skip
        if res_list[i] in need_remove:
            continue

        for j in range(i + 1, len(res_list)):
            # If the comparison object is already in the list that needs to be removed, skip
            ...
            if overlap_box is not None:
                ...
                # Determine which is a small block and which is a large block based on overlapping boxes
                if overlap_box == res_list[i]['bbox']:
                    ...
                    continue  # If the overlapping box does not match any block, skip processing

                if small_res['score'] <= large_res['score']:
                    # If the score of the small block is lower than that of the large block, the small block is the block that needs to be removed
                    if small_res is not None and small_res not in need_remove:
                        # Update the bounds of the chunk to the union of the two
                    ...
                    # If the score of the large block is lower than that of the small block, the large block is the block that needs to be removed, At this time, there is no need to update the boundaries of the small blocks.
                    if large_res is not None and large_res not in need_remove:
                        need_remove.append(large_res)

    # Remove marked element from list
    for res in need_remove:
        res_list.remove(res)
        del res['bbox']  # Delete bbox field

    for res in res_list:
        # Reconstruct the poly of res using bbox
        res['poly'] = [res['bbox'][0], res['bbox'][1], res['bbox'][2], res['bbox'][1],
                       res['bbox'][2], res['bbox'][3], res['bbox'][0], res['bbox'][3]]
        # Delete the bbox of res
        del res['bbox']
...
def remove_overlaps_low_confidence_blocks(combined_res_list, overlap_threshold=0.8):
    ...
    for block in combined_res_list:
        ...
        area = (xmax - xmin) * (ymax - ymin)
        score = block.get('score', 0.5)  # If there is no score field, the default is 0.5
        block_info.append((xmin, ymin, xmax, ymax, area, score, block))

    blocks_to_remove = []
    marked_indices = set()  # Track block indexes marked for deletion

    # Check whether there are 3 or more small blocks inside each block
    for i, (xmin, ymin, xmax, ymax, area, score, block) in enumerate(block_info):
        # If the current block is marked for deletion, skip it
        if i in marked_indices:
            continue

        # Find small internal blocks (Only consider blocks that have not been marked for deletion)
        ...
        # If there are 3 or more small blocks inside
        if len(blocks_inside) >= 2:
            # Calculate the average score of small blocks
            avg_score = sum(s for _, s, _ in blocks_inside) / len(blocks_inside)

            # Compare the score of the large block with the average score of the small block
            if score > avg_score:
                # Keep large blocks and extend their boundaries
                # First mark all small blocks for deletion
                ...
                # Extend the boundaries of the large block to include all small blocks
                ...
                # Keep small blocks and delete large blocks
                blocks_to_remove.append(block)
                marked_indices.add(i)  # Mark the current index as processed
...
def get_res_list_from_layout_res(layout_res, iou_threshold=0.7, overlap_threshold=0.8, area_threshold=0.8):
    ...
    # Detect whether a large block contains multiple small blocks, Merge ocr and table lists for detection
    ...
    # Remove the blocks that need to be deleted
...
def get_vram(device) -> int:
    ...
    # If environment variables are configured,Try to parse and return
    ...
    # Environment variables are not configured or configured incorrectly,Automatically obtain based on device
    total_memory = 1
