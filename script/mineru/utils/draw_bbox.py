...
def draw_bbox_with_number(i, bbox_list, page, c, rgb_config, fill_config, draw_bbox=True):
    ...
    for j, bbox in enumerate(page_data):
        # Make sure every element of bbox is float
        rect = cal_canvas_rect(page, bbox)  # Define the rectangle  
...
def draw_layout_bbox(pdf_info, pdf_bytes, out_path, filename):
    ...
    for i, page in enumerate(pdf_docs.pages):
        ...
        packet = BytesIO()
        # Create a canvas using the dimensions of the original PDF
        ...
        # Add check to ensure overlay_pdf.pagesNot empty
        if len(overlay_pdf.pages) > 0:
            ...
            # Log and continue working on the next page
            # logger.warning(f"layout.pdf: No.{i + 1}Page failed to generate a valid overlay PDF")
            pass
...
def draw_span_bbox(pdf_info, pdf_bytes, out_path, filename):
    ...
    for page in pdf_info:
        ...
        # Construct the remaining useful_list
        # for block in page['para_blocks']:  # spanJust use the results before segmentation merging.
    ...
    for i, page in enumerate(pdf_docs.pages):
        ...
        if len(overlay_pdf.pages) > 0:
            ...
            # Log and continue working on the next page
            # logger.warning(f"span.pdf: No.{i + 1}Page failed to generate a valid overlay PDF")
            pass
...
if __name__ == "__main__":
    ...
    # Call visualization function,Output to the examples directory
